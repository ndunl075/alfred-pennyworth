...
**Gmail senders are deliberately excluded, and that was measured rather than assumed.** Running the same logic over the real 628-message corpus yielded 29 candidates of which 26 were brands (Amazon, Venmo, Pacsun, a16z). No filter rescues it: the connector syncs *unread* mail, which skews entirely to bulk, and not one surviving sender was on a consumer mail domain. The human-looking names were the most dangerous — "Jamie Rivera" arrived from `invites@invites.example`, "Jordan Lee via LinkedIn" from `messaging-digest-noreply@linkedin.com`. Keying a person to a bulk mailer is a false claim about identity, not merely noise; a display name there is the sender's branding.
