...
Read the other way round it is safe and fixes a real defect. Calendar frequently supplies an address with **no** `displayName`, which left people labelled `relative@example.com` — an identifier, not a name anyone uses. Gmail is therefore consulted only for an address Calendar has already vouched for, purely to learn what that person is called; a brand cannot enter that way, because a brand is never a calendar organizer. (An earlier version of this note claimed no sender in the corpus was on a consumer mail domain. That was drawn from the 50 *active* — currently unread — records and was wrong for the full 629: `Jamie Rivera <jamie@example.com>` is real personal correspondence sitting in the archived set, and is exactly the record that now supplies her name.)
