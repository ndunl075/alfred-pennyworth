...
7. **Notebook and polish:** Obsidian/Markdown projection and import plus encrypted local backup/restore are built. A Windows installer script (`scripts/install.ps1`) now covers venv creation, package install, database init, and an optional Task Scheduler entry, touching no provider and previewable with `-WhatIf`. Optional free mobile sync adapter, admin UI, public documentation, and release signing remain unbuilt.
